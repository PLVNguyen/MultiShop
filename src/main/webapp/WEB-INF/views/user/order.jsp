				...
				<div class="col-md-3">
					<div class="nav flex-column nav-pills" role="tablist"
						...
						<a class="nav-link active" id="orders-nav" data-toggle="pill"
							...
							class="nav-link" href="/logout"><i class="fa fa-sign-out-alt"></i>Log out</a>
					</div>
				</div>
				<div class="col-md-9">
										...
										<c:forEach var="item" items="${order}">
											<tr>
												...
												<td><fmt:formatNumber type="number"
														pattern="###,###,###" value="${item.price+price}" /> $</td>
												<td>${item.status?'Transacted':'Waiting'}</td>
												<td><a href="/shop/invoice/${item.name}"
													class="btn btn1 ">Bill</a> <a
													href="/shop/order/delete/${item.name}"
													class="btn btn1 ${item.status?'disabled':''}">Delete</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
